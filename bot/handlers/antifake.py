...
async def on_member_join(event: ChatMemberUpdated):
    if await _is_fake(event.chat.id, event.new_chat_member.user.id, event.bot):
        ...
        await event.bot.send_message(
            event.chat.id,
            f"📵 <b>{name}</b> was removed (fake/restricted number)."
        )
...
async def cmd_antifake(message: Message, is_admin: bool = False):
    ...
    if len(parts) < 2:
        ...
        await message.answer(
            f"📵 <b>Anti-Fake:</b> {status}\n"
            f"Mode: <b>{s['antifake_mode'].upper()}</b>\n"
            f"Prefixes: {', '.join(prefixes) if prefixes else 'None'}\n\n"
            "Usage: /antifake on|off|add [+prefix]|remove [+prefix]|list|mode [blacklist/whitelist]"
        )
        return
    cmd = parts[1].lower()
    if cmd == "on":
        await update_setting(message.chat.id, "antifake", 1)
        await message.answer("✅ Anti-fake <b>enabled</b>.")
    elif cmd == "off":
        await update_setting(message.chat.id, "antifake", 0)
        await message.answer("❌ Anti-fake <b>disabled</b>.")
        ...
        await message.answer("Usage: /antifake on|off|add [+prefix]|remove [+prefix]|list|mode [blacklist/whitelist]")
